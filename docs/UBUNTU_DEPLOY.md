...
The workflows store runtime state in `/opt/business-automations/.data/`.
...
Copy the included service templates into `systemd`. They expect the runtime `.env` at `/opt/business-automations/.env`:
